...
%       hfun --      measurement equation hfun(x, t, hfunpar) --> ici, il faudra adapter au cas de la dette
...
if isfinite(det(PEst))
    ...
    if exist(['C:\code\PSC_OMAP\Code\code_adapté_dette\output\nln_',modelflag,'.txt'],'file');
        nln0=load(['C:\code\PSC_OMAP\Code\code_adapté_dette\output\nln_',modelflag,'.txt']);
        if nlnL<nln0
            save(['C:\code\PSC_OMAP\Code\code_adapté_dette\output\nln',modelflag,'.txt'], 'nlnL', '-ascii','-double');
            save(['C:\code\PSC_OMAP\Code\code_adapté_dette\output\par_',modelflag,'.txt'], 'par', '-ascii','-double');
        end
    else
